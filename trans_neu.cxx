...
Vector3D Uperpi, Uperpe; // drift velocity
...
Field2D psixy;
// 3D evolving fields
Field3D  Te, Ni, Vi, Ti, Nn,Tn, Vn, Nm ,Vmx,Vmy;             // Vi and Vn are paralell velocity
...
BoutReal Nm0,Vm0,alpha;   //alpha = SMBI incidence angle
...
BoutReal term_Vmx,term_Vmy,CF_BC_x0,CF_BC_y0,CF_BC_y1,CF_BC_z0,CF_BC_z1;  // SMBI_LFS
...
bool BScurrent,spitzer_resist,drift,drift_lnB,drift_cv,drift_dia;
...
// Load 2D profiles
  ...
  OPTION(options, drift,false);
  OPTION(options, drift_lnB,false);
  OPTION(options, drift_cv,false);
  OPTION(options, drift_dia,false);
  ...
  OPTION(options, alpha, 0.5);      // read in unit 2*pi
  OPTION(options, term_Vmy, 0.0);      // read in unit 1
  OPTION(options, term_Vmx, 1.0);      // read in unit 1
 ...
 if(!include_curvature)
  ...
  output.write("debug>>>>mesh->geometry is OK!\n"); 
  ...
  Vmy.setLocation(CELL_CENTRE); 
  Vm.setLocation(CELL_CENTRE);
  dVmdt.setLocation(CELL_CENTRE);
  Uperpi.setLocation(CELL_CENTRE);
  Uperpe.setLocation(CELL_CENTRE);
  //////////////// BOUNDARIES ///////////////////////
  // Set BOUNDARiES first here, and then apply them every time in physics run/////
   ...
   Vmx.setBoundary("Vm");
   Vmy.setBoundary("Vm");
   dVmdt.setBoundary("Vm");
   Uperpi.setBoundary("Vm");
   Uperpe.setBoundary("Vm");
  ...
  if(profiles_lasttimestep)
    {
     ...
     BoutReal lstimeNn[NX][NY][NZ],lstimeNm[NX][NY][NZ],lstimeVmx[NX][NY][NZ],lstimeVmy[NX][NY][NZ];  
     // Vn Tn are not evloved in recent version

 	ifstream pFile1,pFile2,pFile3,pFile4,pFile5,pFile6,pFile7,pFile8;
	...
	pFile7.open ("data/lstime_vmx.txt", ios::in );
        pFile8.open ("data/lstime_vmy.txt", ios::in );
	...
	for(int jz=0;jz<mesh->ngz;jz++) 
	  ...
	  Vmy[jx][jy][jz] = lstimeVmy[mesh->XGLOBAL (jx)][mesh->YGLOBAL (jy)][jz];
        }
  ...
  output.write("debug>>>>Initialization of Profiles is OK!\n");
  ...
  if(extsrcs_balance_diffusion)
   ...
   output.write("debug>>>>Hmode-diffusion is OK!\n"); 
 ...
 if (BScurrent)
  ...
  if(drift)
  {
    ...
    if (drift_lnB) Uperpi += (Ti/ZZ/B3d)*(B0vec^Grad(lnB));
    if (drift_cv)  Uperpi += (Ti/ZZ/B3d)*b0xcv;
    if (drift_dia) Uperpi += (1.0/ZZ/B3d/Ni)*(B0vec^Grad(Pi));
    Uperpi *= Ti_x*tbar/Lbar/Lbar/bmag;
    mesh->communicate(Uperpi);
    Uperpi.applyBoundary();
    dump.add(Uperpi,"Uperpi",1);

    if (drift_lnB) Uperpe -= (Te/ZZ/B3d)*(B0vec^Grad(lnB));
    if (drift_cv)  Uperpe -= (Te/ZZ/B3d)*b0xcv;
    if (drift_dia) Uperpe -= (1.0/ZZ/B3d/Ni)*(B0vec^Grad(Pe));
    Uperpe*=Te_x*tbar/Lbar/Lbar/bmag;
    mesh->communicate(Uperpe);
    Uperpe.applyBoundary();
    dump.add(Uperpe,"Uperpe",1);
  }// need two code blocks 
  ...
  dump.add(dVmdt,"dVmdt",1); 
  Nm=ret_const_flux_BC(Nm, Nm0);
  Vmx=ret_const_flux_BC(Vmx, -abs(Vm0*sin(alpha*1.*PI)) );
  Vmy=ret_const_flux_BC(Vmy, Vm0*cos(alpha*1.*PI));
  output<<"debug>>>> check SMBI_LFS="<<SMBI_LFS<<endl;
  output<<"debug>>>> check NX, NY, NZ="<<NX<<","<<NY<<","<<NZ<<endl;
  output<<"debug>>>> check mesh->ngx, ngy="<<mesh->ngx<<","<<mesh->ngy<<endl;
  output<<"debug>>>> check mesh->GlobalX @ guard cell="<<mesh->GlobalX(0)<<","<<mesh->GlobalX(NX-1)<<endl;
  output<<"debug>>>> check Nm @ guard cell="<<max(Nm)<<endl;
  output<<"debug>>>> check Vm @ guard cell="<<max(abs(Vmx))<<endl;
...
int physics_run(BoutReal t)
{
  ...
  mesh->communicate(Nn);
  mesh->communicate(Nm,Vm);
  mesh->communicate(Vmx,Vmy);
  ...
  Tn=temp_Ti;
  Vmx=Vm.x; //updata Vm.x -> Vmx
  Vmy=Vm.y; //updata Vm.y -> Vmy
  if(SMBI_LFS)
  {
  Nm=ret_const_flux_BC(Nm, Nm0);
  Vmx=ret_const_flux_BC(Vmx, -abs(Vm0*sin(alpha*1.*PI)) );
  ...
  else
 {
  ...
  Vmy=ret_const_flux_BC(Vmy,0.0);
  Vm.z=0.0;
  }
  Vm.x=term_Vmx*Vmx; //updata Vmx -> Vm.x
  Vm.y=term_Vmy*Vmy; //updata Vmy -> Vm.y
 ...
 //Dissociation rate of molecules
  ...
  //Diffc_nn_perp calculated once for SBC and again later
...
//************************************
  ...
  ddt(Ni) =   Diff_ni_perp*Delp2(Ni)
            - Ni*Grad_par(Vi)
            - V_dot_Grad(Uperpi,Ni)+Ni*Div(Uperpi)
            - Vpar_Grad_par(Vi,Ni)
            + Si_p; 
...
//************************************
  ...
  ddt(Te) =  0.6667*kappa_Te*Grad2_par2(Te)/temp_Ni
            // + 0.6667*Grad_par(kappa_Te)*Grad_par(Te)/temp_Ni
             ...
             -V_dot_Grad(Uperpe,Te)-0.6667*Te*Div(Uperpe);
...
//************************************
  ...
  //ddt(Ti)=0.0;
   ...
   ddt(Ti) = - Vpar_Grad_par(Vi,Ti)
             ...
             - nu_ionz*Ti
             - 0.6667*nu_CX*(Ti-Tn)
             -V_dot_Grad(Uperpi,Ti)-0.6667*Ti*Div(Uperpi) ;
 ...
 if (BScurrent)
    {
      ...
      mesh->communicate(Jpar_BS0);
      Jpar_BS0.applyBoundary();
    }// need two code blocks about BScurrent(intialising and updating)
 if(drift)
 {
    B3d=Field3D( mesh->Bxy);
    lnB=Field3D( log(B3d) );
    ...
    mesh->communicate(Uperpe);
    Uperpe.applyBoundary();
 }// need two code blocks about drift(intialising and updating)
    ...
    for (int jx=0;jx<mesh->ngx;jx++)
        {
           ...
           for (int jy=0;jy<mesh->ngy;jy++)
	      {
		     ...
		     if(jx>=NX-2 && y_glb>=CF_BC_y0 && y_glb<=CF_BC_y1 && z_glb>=CF_BC_z0 && z_glb<=CF_BC_z1) 
		       { 
